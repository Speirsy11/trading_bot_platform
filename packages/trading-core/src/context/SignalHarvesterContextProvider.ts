...
export class SignalHarvesterContextProvider implements StrategyContextProvider {
  ...
  async getEvents(query: { topic: string; from: Date; to: Date; limit?: number }): Promise<ExternalContextEvent[]> {
    ...
    if (!response.ok) throw new Error(`Signal Harvester context request failed: ${response.status}`);
